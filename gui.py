...
from generator import ForgeDepGenerator
...
class ModDependencyGUI:
    ...
    def __init__(self, root):
        self.root = root
        self.root.title(f"Modrinth MOD 依赖生成器 v{__version__}")
        # 增加窗口宽度，让右侧配置信息显示更清晰
        ...
        # 存储版本列表
        self.mod_versions = []  # 筛选后的版本列表
        self.all_mod_versions = []  # 所有版本列表（用于重新筛选）
        self.selected_mod_version = None
        # 存储图片引用，防止被垃圾回收
        self.image_cache = {}
        # 滑动加载相关
        self.current_query = ""
    ...
    def create_widgets(self):
        ...
        title_label = ctk.CTkLabel(
            search_frame, text="搜索 MOD", font=ctk.CTkFont(size=16, weight="bold")
        )
        title_label.grid(row=0, column=0, columnspan=5, pady=(10, 5))

        # 搜索关键词输入
        ctk.CTkLabel(search_frame, text="MOD 名称:").grid(
            row=1, column=0, sticky=tk.W, padx=5, pady=5
        )
        self.search_entry = ctk.CTkEntry(
            search_frame, width=300, placeholder_text="输入 MOD 名称..."
        )
        self.search_entry.grid(row=1, column=1, padx=5, pady=5)
        self.search_entry.bind("<Return>", lambda e: self.search_mods())

        # 加载器选择
        ctk.CTkLabel(search_frame, text="加载器:").grid(
        ...
        search_btn = ctk.CTkButton(
            search_frame, text="搜索", command=self.search_mods, width=100
        )
        ...
        results_title = ctk.CTkLabel(
            left_frame, text="搜索结果", font=ctk.CTkFont(size=14, weight="bold")
        )
        ...
        try:
            bg_color = (
                ctk.ThemeManager.theme["CTkFrame"]["fg_color"][1]
                if ctk.get_appearance_mode() == "dark"
                else ctk.ThemeManager.theme["CTkFrame"]["fg_color"][0]
            )
        except Exception:
        ...
        def _on_canvas_mousewheel(event):
            ...
            try:
                ...
                if widget_under_mouse:
                    # 排除详情文本和配置文本区域
                    if (
                        hasattr(self, "detail_text")
                        and widget_under_mouse == self.detail_text
                    ):
                        return  # 让 CTkTextbox 自己处理
                    if (
                        hasattr(self, "result_text")
                        and widget_under_mouse == self.result_text
                    ):
                        return  # 让 CTkTextbox 自己处理
                    ...
                    while current:
                        if (
                            current == canvas
                            or current == list_frame
                            or current == self.results_container
                        ):
                            # Windows 和 Mac
        ...
        detail_title = ctk.CTkLabel(
            detail_frame, text="MOD 详情", font=ctk.CTkFont(size=12, weight="bold")
        )
        ...
        config_title = ctk.CTkLabel(
            right_frame, text="生成的配置", font=ctk.CTkFont(size=14, weight="bold")
        )
        ...
        generate_btn = ctk.CTkButton(
            row1, text="生成配置", command=self.generate_config, width=100
        )
        ...
        ctk.CTkLabel(row2, text="MOD 版本:").pack(side=tk.LEFT, padx=5)
        self.mod_version_var = tk.StringVar(value="最新版本")
        # 使用按钮+标签显示当前选择的版本，点击后弹出选择窗口
        self.mod_version_display = ctk.CTkButton(
            row2,
            text="最新版本",
            command=self.show_version_selection_window,
            width=200,
            fg_color=("gray75", "gray25"),
            hover_color=("gray60", "gray40"),
        )
        self.mod_version_display.pack(side=tk.LEFT, padx=5)

        # 刷新版本列表按钮
        refresh_versions_btn = ctk.CTkButton(
            row2, text="刷新版本", command=self.load_mod_versions, width=80
        )
        refresh_versions_btn.pack(side=tk.LEFT, padx=5)

        # 第三行：配置模式和依赖类型
        row3 = ctk.CTkFrame(version_frame)
        row3.pack(fill=tk.X, pady=2)
        ctk.CTkLabel(row3, text="配置模式:").pack(side=tk.LEFT, padx=5)
        self.config_mode_var = tk.StringVar(value="full")
        config_mode_combo = ctk.CTkComboBox(
            row3,
        ...
        ctk.CTkLabel(row3, text="依赖类型:").pack(side=tk.LEFT, padx=5)
        self.dependency_type_var = tk.StringVar(value="implementation")
        dependency_type_combo = ctk.CTkComboBox(
            row3,
        ...
        self.result_text = ctk.CTkTextbox(
            right_frame, font=ctk.CTkFont(family="Consolas", size=13), wrap=tk.NONE
        )  # 不自动换行，保持代码格式
        ...
        save_btn = ctk.CTkButton(
            bottom_frame, text="保存到文件", command=self.save_to_file, width=120
        )
        save_btn.pack(side=tk.LEFT, padx=5)

        # 清空按钮
        clear_btn = ctk.CTkButton(
            bottom_frame, text="清空结果", command=self.clear_result, width=120
        )
        ...
        status_bar = ctk.CTkLabel(
            bottom_frame, textvariable=self.status_var, anchor="w"
        )
    ...
    def append_search_results(self, results, has_more):
        ...
        if not results and len(self.search_results) == 0:
            ...
            # 检查是否包含中文字符
            has_chinese = any(
                "\u4e00" <= char <= "\u9fff" for char in self.current_query
            )
            if has_chinese:
                messagebox.showinfo(
                    "提示",
        ...
        if has_more:
            self.status_var.set(
                f"已加载 {len(self.search_results)} 个结果（滚动到底部加载更多）"
            )
        else:
    ...
    def create_mod_item(self, mod, index):
        ...
        img_label = ctk.CTkLabel(
            item_frame, text="", width=64, height=64, fg_color=("gray80", "gray30")
        )  # 使用主题颜色
        ...
        def load_image():
            ...
            if icon_url and not (
                icon_url.startswith("http://") or icon_url.startswith("https://")
            ):
                try:
            ...
            if icon_url:
                ...
                if photo:
                    ...
                    if hasattr(item_frame, "img_label") and not (
                        hasattr(item_frame, "_image_set") and item_frame._image_set
                    ):
        ...
        def on_item_click(event=None):
            ...
            item_frame.configure(
                border_width=2,
                border_color=ctk.ThemeManager.theme["CTkButton"]["fg_color"][1],
            )
...
"""
    ...
    def set_detail_image(self, photo):
        ...
        if hasattr(self, "detail_image_label"):
            ...
            self.detail_image_label.configure(
                image=photo, text="", width=120, height=120
            )
    ...
    def on_loader_change(self, choice):
        """加载器改变时的回调"""
        # 如果已经选择了MOD，重新加载版本列表以应用新的加载器筛选
        if self.selected_mod:
            self.load_mod_versions()

    def load_mod_versions(self):
        """加载 MOD 版本列表（根据当前选择的加载器筛选）"""
        if not self.selected_mod:
            return

        self.status_var.set("正在加载版本列表...")
        current_loader = self.loader_var.get()

        def do_load():
            try:
                # 获取所有版本（不筛选）
                all_versions = self.api.get_all_project_versions(
                    self.selected_mod["slug"]
                )
                # 根据当前选择的加载器筛选版本
                filtered_versions = self.filter_versions_by_loader(
                    all_versions, current_loader
                )
                self.root.after(
                    0, self.update_version_list, filtered_versions, all_versions
                )
            except Exception as e:
                error_msg = str(e)
                self.root.after(
                ...
                self.root.after(0, lambda: self.status_var.set("加载版本失败"))
    ...
    def filter_versions_by_loader(self, versions, loader):
        ...
        for v in versions:
            version_loaders = v.get("loaders", [])
            # 检查版本是否支持当前选择的加载器
            if loader_lower in [ldr.lower() for ldr in version_loaders]:
                filtered.append(v)

        return filtered

    def update_version_list(self, filtered_versions, all_versions=None):
        """更新版本列表"""
        # 保存所有版本（用于后续筛选）和筛选后的版本
        if all_versions is not None:
            self.all_mod_versions = all_versions  # 保存所有版本
        self.mod_versions = filtered_versions  # 保存筛选后的版本

        if not filtered_versions:
            self.mod_version_display.configure(text="无可用版本")
            self.mod_version_var.set("无可用版本")
            current_loader = self.loader_var.get()
            self.status_var.set(f"无可用版本（加载器: {current_loader}）")
            return

        # 更新显示按钮文本
        self.mod_version_display.configure(text="最新版本")
        self.mod_version_var.set("最新版本")
        self.selected_mod_version = None

        current_loader = self.loader_var.get()
        total_count = (
            len(self.all_mod_versions)
            if hasattr(self, "all_mod_versions")
            else len(filtered_versions)
        )
        self.status_var.set(
            f"已加载 {len(filtered_versions)} 个版本（加载器: {current_loader}，共 {total_count} 个）"
        )

    def show_version_selection_window(self):
        """显示版本选择窗口（显示根据加载器筛选后的版本）"""
        if not self.mod_versions:
            messagebox.showinfo("提示", "请先选择一个 MOD 并等待版本列表加载完成")
            return
        ...
        version_window.title(f"选择 MOD 版本（加载器: {current_loader}）")
        ...
        # 计算主窗口的中心位置
        self.root.update_idletasks()  # 确保主窗口位置已更新
        ...
        # 计算弹窗应该出现的位置（主窗口中心）
        popup_x = main_x + (main_width // 2) - (window_width // 2)
        popup_y = main_y + (main_height // 2) - (window_height // 2)

        # 设置窗口位置和大小
        version_window.geometry(f"{window_width}x{window_height}+{popup_x}+{popup_y}")
        version_window.transient(self.root)  # 设置为子窗口

        # 标题（显示当前加载器）
        title_text = f"选择 MOD 版本（加载器: {current_loader}）"
        title_label = ctk.CTkLabel(
            version_window, text=title_text, font=ctk.CTkFont(size=16, weight="bold")
        )
        title_label.pack(pady=10)

        # 显示筛选信息
        if hasattr(self, "all_mod_versions") and len(self.all_mod_versions) > len(
            self.mod_versions
        ):
            filter_info = ctk.CTkLabel(
                version_window,
                text=f"显示 {len(self.mod_versions)} 个版本（共 {len(self.all_mod_versions)} 个，已按加载器筛选）",
                font=ctk.CTkFont(size=10),
                text_color=("gray50", "gray70"),
            )
            filter_info.pack(pady=(0, 5))

        # 创建滚动框架（限制高度，支持滚轮滚动）
        scroll_frame = ctk.CTkScrollableFrame(version_window, height=300)
        ...
        latest_btn = ctk.CTkButton(
            scroll_frame,
            text="最新版本",
            command=select_latest,
            width=450,
            fg_color=("gray70", "gray30")
            if self.selected_mod_version is None
            else ("gray75", "gray25"),
        )
        latest_btn.pack(pady=5, padx=10)

        # 添加版本选项
        for v in self.mod_versions:
            version_number = v.get("version_number", "未知")
            ...
            # 创建版本按钮（使用闭包捕获变量）
            def make_version_selector(ver_num, ver_date):
                def select_version():
                    self.selected_mod_version = ver_num
                    display_text = f"{ver_num} ({ver_date})"
                    self.mod_version_var.set(display_text)
                    self.mod_version_display.configure(text=display_text)
                    version_window.destroy()

                return select_version

            version_btn = ctk.CTkButton(
                scroll_frame,
                text=f"{version_number} ({formatted_date})",
                command=make_version_selector(version_number, formatted_date),
                width=450,
                fg_color=("gray70", "gray30")
                if self.selected_mod_version == version_number
                else ("gray75", "gray25"),
                anchor="w",
            )
            version_btn.pack(pady=2, padx=10)

            # 添加详细信息标签
            info_text = f"  游戏版本: {', '.join(game_versions[:3])}" + (
                f" 等{len(game_versions)}个" if len(game_versions) > 3 else ""
            )
            info_text += f" | 加载器: {', '.join(loaders)}"
        ...
        close_btn = ctk.CTkButton(
            version_window, text="关闭", command=version_window.destroy, width=100
        )
        close_btn.pack(pady=10)

    def on_mod_version_select(self, choice):
        """MOD 版本选择回调（保留以兼容旧代码）"""
        if choice == "最新版本":
            self.selected_mod_version = None
        else:
            # 提取版本号（去掉日期部分）
            version_number = choice.split(" (")[0]
            self.selected_mod_version = version_number
    ...
    def generate_config(self):
        ...
        mod_version = self.selected_mod_version  # 使用选中的版本
